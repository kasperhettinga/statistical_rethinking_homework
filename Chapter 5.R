...
mu <- link(model5Ex1,data=list(T=T_seq))
mu.mean <- apply(mu,2,mean)
mu.PI <- apply(mu,2,PI,prob=0.91)
plot(T~W,data=d,col=rangi2)
...
mu <- link(model5Ex2,data=list(G=G_seq))
mu.mean <- apply(mu,2,mean)
mu.PI <- apply(mu,2,PI,prob=0.91)
...
sim_TW_data <- data.frame(T=seq(from=-3, to=3, length.out=30), G=0)
sim_TW <- sim(model5Ex3,data=sim_TW_data)
plot(sim_TW_data$T,colMeans(sim_TW),type="l")
shade(apply(sim_TW,2,PI), sim_TW_data$T)

sim_TG_data <- data.frame(G=seq(from=-3, to=3, length.out=30), T=0)
sim_TG <- sim(model5Ex3,data=sim_TG_data)
plot(sim_TG_data$G,colMeans(sim_TG),type="l")
shade(apply(sim_TG,2,PI), sim_TG_data$G)
